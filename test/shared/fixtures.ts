...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy migrator
  const migrator = await deployContract(wallet, DipoleMigrator, [factoryV1.address, router01.address], overrides)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IDipolePair.abi), provider).connect(wallet)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IDipolePair.abi), provider).connect(wallet)
