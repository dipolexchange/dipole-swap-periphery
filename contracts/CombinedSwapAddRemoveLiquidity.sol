...
contract CombinedSwapAddRemoveLiquidity {
    using SafeMath for uint;

    IDipoleFactory public immutable factory;
    IDipoleRouter01 public immutable router;
    IWETH public immutable weth;

    constructor(IDipoleFactory factory_, IDipoleRouter01 router_, IWETH weth_) public {
        factory = factory_;
        router = router_;
        weth = weth_;
    }
    ...
    function calculateSwapInAmount(uint reserveIn, uint userIn) public pure returns (uint) {
        return Babylonian.sqrt(reserveIn.mul(userIn.mul(399000000) + reserveIn.mul(399000625))).sub(reserveIn.mul(19975)) / 19950;
    }
    ...
    function _swapExactTokensAndAddLiquidity(
        ...
        uint swapInAmount;
        {
            (uint reserveIn,) = DipoleLibrary.getReserves(address(factory), tokenIn, otherToken);
    ...
    function _removeLiquidityAndSwap(
        ...
        address pair = DipoleLibrary.pairFor(address(factory), undesiredToken, desiredToken);
